...
║    from pomodoro import PomodoroCog                              ║
║    await bot.add_cog(PomodoroCog(bot))   # trong on_ready()     ║
║                                                                  ║
║  Lệnh slash:                                                     ║
...
class PomodoroCog(commands.Cog):
    ...
    Inject `add_study_time` và `safe_send_dm` từ bot.py qua hàm khởi tạo.
    """
    ...
    def __init__(
        ...
        # lịch sử: member_id → {'total_rounds': int, 'total_minutes': int}
    ...
    async def pomo_start(
        ...
        await interaction.response.send_message(
            ...
            f'Tin nhắn bên dưới sẽ tự cập nhật mỗi 30 giây.',
            ephemeral=True
        )
    ...
    async def pomo_stats(self, interaction: discord.Interaction):
        member  = interaction.user
        history = self._history.get(member.id, {})
        ...
        today_rounds   = history.get('today_rounds', 0)
    ...
    async def _run_personal(self, sess: PomodoroSession):
        ...
        try:
            while sess.current_round <= sess.total_rounds:
                ...
                # Cộng XP Pomodoro
                self._award_pomodoro_xp(sess.member.id, sess.member.display_name)

                sess.completed_rounds += 1
                self._update_history(sess.member.id, sess, partial=True)
                ...
                await self._send_dm(
                    ...
                    f'⏰ Nghỉ `{sess.break_minutes} phút` — vươn vai, uống nước nhé!\n'
    ...
    async def _run_group(self, grp: GroupSession):
        ...
        try:
            while grp.current_round <= grp.total_rounds:
                ...
                # Lưu thời gian cho tất cả thành viên
                for sess in list(grp.members.values()):
                    self._add_study(
                        sess.member.id,
                        sess.member.display_name,
                ...
                for sess in list(grp.members.values()):
                    ...
                    await self._send_dm(
                        ...
                        f'⏰ `{grp.break_minutes} phút` — cả nhóm nghỉ cùng nhau!'
                    )
    ...
    def _build_personal_embed(self, sess: PomodoroSession) -> str:
        ...
        return (
            ...
            f'⏰ Còn lại: `{sess.format_remaining()}`\n'
            f'`{sess.progress_bar}` {int((1 - sess.phase_remaining / (sess.work_minutes * 60 if sess.phase == "work" else sess.break_minutes * 60)) * 100)}%\n'
    ...
    def _award_pomodoro_xp(self, member_id: int, member_name: str):
        """Cộng XP thưởng cho 1 vòng Pomodoro hoàn thành."""
        # Dùng add_study_time với 0 giây chỉ để trigger XP logic là không đúng.
        # Thay vào đó, lưu trực tiếp qua add_study_time với 1 giây dummy để đảm bảo
        # user tồn tại trong DB, rồi patch XP riêng.
        # Tuy nhiên, do cần đơn giản và không expose thêm hàm, ta dùng add_study_time
        # với XP_PER_ROUND giây (1 phút) như một khoản thưởng tượng trưng.
        # Thực tế XP bonus nên được xử lý trong bot.py nếu muốn chính xác.
        pass   # XP đã được tính qua add_study_time với work_minutes * 60
    ...
    def _update_history(
        ...
        """Cập nhật lịch sử Pomodoro và trả về summary."""
        today       = datetime.now().strftime('%Y-%m-%d')
        h           = self._history.setdefault(member_id, {
            'total_rounds': 0, 'total_minutes': 0,
            'total_xp': 0, 'best_streak': 0,
            'today_rounds': 0, 'today_date': today,
            'current_streak': 0,
        })
...
def create_pomodoro_cog(bot, add_study_time, safe_send_dm, format_time):
    ...
    """
    return PomodoroCog(bot, add_study_time, safe_send_dm, format_time)